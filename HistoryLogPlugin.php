...
//     		'after_save_record',  
  ...
  function hookAfterSaveExhibitPage($args)
  {
    ...
    // Pas de post => appel par autre chose qu'une création manuelle.
    if (! $args['post']) {
      return;
    }
    
    $db = $this->_db;   
    $query = "INSERT INTO `$db->HistoryLogEntry` VALUES(null, 'Exhibit Page', $record_id, 0, $user->id, '$operation', null)";      
    $db->query($query);
  } 
  
  function hookAfterDeleteExhibitPage($args)
  {
    $record = $args['record'];
    $record_id = $record->id; 
    $db = $this->_db;    
    $user = current_user();
    $query = "INSERT INTO `$db->HistoryLogEntry` VALUES(null, 'Exhibit Page', $record_id, 0, $user->id, 'delete', null)";      
    $db->query($query);
  }  
      
  function hookAfterSaveSimplePagesPage($args)
  {
    $record = $args['record'];
    $insert = $args['insert'];
    ...
    $db = $this->_db;    
    $query = "INSERT INTO `$db->HistoryLogEntry` VALUES(null, 'Simple Page', $record_id, 0, $user->id, '$operation', null)";      
    $db->query($query);
  }
  
  function hookAfterDeleteSimplePagesPage($args)
  {
    $record = $args['record'];
    $record_id = $record->id; 
    $db = $this->_db;    
    $user = current_user();
    $query = "INSERT INTO `$db->HistoryLogEntry` VALUES(null, 'Simple Page', $record_id, 0, $user->id, 'delete', null)";      
    $db->query($query);
  }  
    ...
    public function hookInstall()
    {
        ...
        CREATE TABLE IF NOT EXISTS `{$db->HistoryLogEntry}` (
            ...
            `record_type` enum('Item', 'Collection', 'File', 'Simple Page', 'Exhibit Page', 'Transcription') NOT NULL,
